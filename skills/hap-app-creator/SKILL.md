...
5. **收尾与按需修复**：展示完整清单；若有失败项，提示用户先打开应用查看，再由用户决定是否用 hap-app-editor 在原位修复。
...
- **工作目录**：`cd` 到本 skill 目录（`skills/hap-app-creator/`）再运行 `python3 -m scripts ...`，这样 `scripts` 包可被导入。
...
- build 阶段则**永远从干净 design 一次性整跑到底**——中途某步（某条工作流/某个视图）失败只会被记录，流程继续，**绝不分阶段重跑、绝不自动改 design 重建**。失败项在全部跑完后用 hap-app-editor 按真实 id 在原位修复（见 Step 5）。
...
2. **必须用多个 subAgent 并行生成其余几块**：地基锁定后，**同时派出多个子代理（一次性发起多个 Agent 调用，并行运行）**。每个子代理负责一块、各自只放自己那部分顶层键、**不要重复 `app`**，把表名/字段名严格对齐地基：
   - `02-views.design.json` → `views`
   - `03-roles.design.json` → `roles`
   - `04-workflows.design.json` → `workflows` + `custom_actions`（这块最重，单独一个子代理）
   - `05-pages.design.json` → `custom_pages` + `chatbots`
   > 子代理任务要自包含：把**地基里的表名/字段名/选项清单**连同对应 `references/` 一并交给它，让它无需回头读地基文件即可生成。各块互不依赖，所以可真正并行。**这几块各算一项任务**，子代理回来一块就标记 `completed` 并告知用户进度。
   >
   > 平台映射：
...
> ≤ 5 表可单份 `app.design.json`；> 5 表必须拆分。拆分时务必：①只有地基含 `app`；②各片顶层键不相交；③跨片引用的表名/字段名必须与地基**完全一致**（合并后会整体校验兜底）。
...
> [!CAUTION]
> **修复绝不回到本 skill 的 build——不重建、不重跑任何阶段。** 用户选「自动修复」时，**改用 [hap-app-editor](../hap-app-editor/SKILL.md) skill**，对每个失败元素**按它的真实 id 在原位精确修正**：
...
- 已经有应用、只想改一两个字段——用 hap-app-editor skill 或直接用 `hap` 命令，不必走完整建应用流程。
