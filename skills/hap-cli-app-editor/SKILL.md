...
执行逻辑都在本 skill 的 `scripts/` 里（一套自包含、stdlib-only 的框架，通过 subprocess 调真实 `hap` 二进制；不依赖、不复用 hap-cli-app-creator）。你把修改表达成一个结构化的 **edit-spec**（JSON），框架负责校验、读取应用实时结构解析逻辑名、预演、执行。
