...
| CLI | **hap-cli-app-creator** | 从一句业务需求一站式建出**真实可用、带示例数据**的明道云应用 |
...
| API | **hap-api-frontend-project** | HAP + 前端项目完整搭建指南：后台配置、前端项目结构、API 集成与数据渲染 |
| API | **hap-api-view-plugin** | 开发明道云 HAP 自定义视图插件（mdye）：开发工作流、API 用法与最佳实践 |
...
npx skills add mingdaocom/hap-skills --skill hap-api-as-database
npx skills add mingdaocom/hap-skills --skill hap-api-frontend-project
npx skills add mingdaocom/hap-skills --skill hap-api-view-plugin
```
...
适合「直接调 HAP 接口」：讲清鉴权配置（Appkey / Sign）、接口调用、筛选器（Filter）用法、记录的查询与增删改等。若已配置 HAP MCP，可自动从 MCP 配置中提取鉴权密钥。是另外几个 API 技能的接口基础。

#### hap-api-as-database — 用 HAP 当后端数据库

适合「用 HAP 作后台、前后端分离建站」的总览入口：当你想做企业官网、内容管理系统、数据展示平台时，给出以 HAP 为后台内容管理系统的整体搭建思路，并衔接到 `hap-api-frontend-project` 的详细指南。

#### hap-api-frontend-project — HAP + 前端项目搭建

适合「把网站真正搭起来」：提供从 HAP 后台配置、前端项目结构、API 集成到数据渲染的完整指南，是 `hap-api-as-database` 思路的落地版。

#### hap-api-view-plugin — 自定义视图插件开发
...
开发一个 HAP 自定义视图插件                 # → hap-api-view-plugin
```
